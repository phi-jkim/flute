...
#ifndef OPTIMIZED_W3_KERNEL_CUH
#define OPTIMIZED_W3_KERNEL_CUH
...
// ---- W3 (3-bit) LUT dequant, nibble-selector variant: each 3-bit code is
// stored offline in its own nibble (32 codes = 16B unit, same bytes as W4),
// so a fetched 32-bit word IS a pair of prmt selectors -- no window
// extraction, no selector spread. prmt reads only bits 15:0 of the control
// operand and codes 0-7 keep every nibble's mode bit clear, so the low half
// needs no masking at all. 8 values = 1 SHR + 8 PRMT. ----
struct W3Lut { unsigned lo0, lo1, hi0, hi1; };

__device__ inline FragB dequant_w3(unsigned sel, const W3Lut L) {
  const unsigned lo = __byte_perm(L.lo0, L.lo1, sel);
  const unsigned hi = __byte_perm(L.hi0, L.hi1, sel);
  unsigned v0 = __byte_perm(lo, hi, 0x5140);   // halfs (c0, c1)
  unsigned v1 = __byte_perm(lo, hi, 0x7362);   // halfs (c2, c3)
  FragB frag_b;
...
>
__global__ void OptimizedW3(
  const int4* __restrict__ A, // fp16 input matrix of shape mxk
  const int4* __restrict__ B, // 3bit LUT-quantized weights, one code per nibble (16B units)
        int4* __restrict__ C, // fp16 output buffer of shape mxn
  const int4* __restrict__ s, // fp16 quantization scales of shape (k/groupsize)xn
  const W3Lut lut, // 8-entry fp16 dequant table as lo/hi byte planes
  ...
  auto matmul = [&] (int k) {
    ...
    // W3 nibble layout: word j of the unit holds perm'd elements 8j..8j+7 as
    // 8 nibbles; low/high 16 bits are the two fragment selectors directly.
    #pragma unroll
    for (int j = 0; j < 4; j++) {
      const unsigned qw = (unsigned) frag_b_quant[k % 2][j];
      FragB frag_b0 = dequant_w3(qw, lut);
      // If there are no groups, we can just scale the final output once and can avoid doing so for each weight.
      if (group_blocks != -1)
        scale(frag_b0, frag_s[k % 2][j], 0);
      FragB frag_b1 = dequant_w3(qw >> 16, lut);
      if (group_blocks != -1)
...
#define CALL_IF(THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, GROUP_BLOCKS) \
    ...
    cudaFuncSetAttribute( \
      OptimizedW3<THREADS, THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, STAGES, GROUP_BLOCKS>, \
...
int optimized_w3_cuda(
  const void* A,
  const void* B,
        void* C,
        void* s,
  unsigned long long lut_lo,
  unsigned long long lut_hi,
  int prob_m,
  int prob_n,
  int prob_k,
  void* workspace,
  ...
  const W3Lut lut = {
    (unsigned)(lut_lo & 0xFFFFFFFFull), (unsigned)(lut_lo >> 32),
    (unsigned)(lut_hi & 0xFFFFFFFFull), (unsigned)(lut_hi >> 32)
  };
